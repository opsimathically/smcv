...
| P4-RES-001 | Medium | Backup creation and verification both perform expensive Argon2/archive work; independent unbounded request concurrency could exhaust CPU or memory. | Phase 4 initially made both operations share four bounded slots. The later ten-pass operational review superseded that calibration with one independent archive slot plus 256 MiB logical/KDF ceilings, preserving all route body/time bounds without consuming password-login capacity. |
