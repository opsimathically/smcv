...
| OPS-001/OPS-002 database behavior | Database and snapshots are atomic mode-`0600` creations; snapshots use a same-directory partial, no-follow SQLite connection, durable no-overwrite publication, and no residual partial name. Unsafe files fail closed. WAL, FULL sync, foreign keys, secure deletion, trusted-schema off, a five-second busy bound, 16 MiB SQL value limit, quick integrity check, and no-overwrite online backup are executable tests. |
