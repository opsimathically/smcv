...
| BACKUP-015 | Server artifacts use opaque UUID names, a non-symlink same-owner mode-0700 directory, mode-0600 files, 32-job quota, terminal-artifact 15-minute expiry, explicit delete, and durable download-started/status state. Pending/running work becomes failed/interrupted on process restart. The operational review renamed the transport observation because an HTTP response cannot prove client-side completion or custody. |
